(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))r(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const a of i.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function n(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerPolicy&&(i.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?i.credentials="include":o.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(o){if(o.ep)return;o.ep=!0;const i=n(o);fetch(o.href,i)}})();function we(e){if(!e)return"";const t=new Date(e),r=Math.floor((new Date-t)/1e3);let o=r/31536e3;return o>1?Math.floor(o)+"y ago":(o=r/2592e3,o>1?Math.floor(o)+"mo ago":(o=r/86400,o>1?Math.floor(o)+"d ago":(o=r/3600,o>1?Math.floor(o)+"h ago":(o=r/60,o>1?Math.floor(o)+"m ago":Math.floor(r)+"s ago"))))}function xe(e="news-grid"){const t=document.getElementById(e);if(t){t.innerHTML="";for(let n=0;n<4;n++){const r=document.createElement("article");r.className="skeleton-card",r.innerHTML=`
        ...
        `,t.appendChild(r)}}}function Ce(e,t,n="news-grid"){const r=document.getElementById(n);if(r&&(r.innerHTML=`
    ...
    `,t)){const o=document.getElementById(`${n}-retry-btn`);o&&o.addEventListener("click",t)}}function Ie(e,t="news-grid"){const n=document.getElementById(t);if(!n)return;const r=e?` for "${e}"`:"";n.innerHTML=`
    ...
    `}function Ee(e,t="news-grid"){const n=document.getElementById(t);if(!n)return;if(n.innerHTML="",!e||e.length===0){n.innerHTML='<p style="grid-column: 1 / -1; text-align: center; color: var(--text-secondary);">No articles found.</p>';return}const r=JSON.parse(localStorage.getItem("saved_articles")||"[]");e.forEach(o=>{var f;const i=document.createElement("article");i.className="news-card",i.tabIndex=0;const a=o.urlToImage||o.image||"https://images.unsplash.com/photo-1585829365295-ab7cd400c167?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",s=o.category||"News",d=`badge-${s.toLowerCase()}`,h=((f=o.source)==null?void 0:f.name)||"Unknown Source",g=we(o.publishedAt),w=o.title||"No Title",p=o.description||"No description available.",l=o.url||"#",u=r.some(v=>v.url===l&&v.title===w);i.innerHTML=`
  ...
  `};function k(e,t=""){const n=ae[e]||ae.cloud;return t?n.replace(/class="([^"]*)"/,`class="$1 ${t}"`):n}function fe(e){const t=Number(e);return t===0?"sun":[1,2,3].includes(t)||[45,48].includes(t)?"cloud":[51,53,55,56,57].includes(t)?"cloud-drizzle":[61,63,65,66,67,80,81,82].includes(t)?"cloud-rain":[71,73,75,77,85,86].includes(t)?"snowflake":[95,96,99].includes(t)?"cloud-lightning":"cloud"}function se(e){const t=Number(e);return t===0?"Clear Skies":t===1?"Mainly Clear":t===2?"Partly Cloudy":t===3?"Overcast":t===45||t===48?"Foggy":t===51||t===53||t===55?"Drizzle":t===61||t===63||t===65?"Rainy":t===66||t===67?"Freezing Rain":t===71||t===73||t===75?"Snowy":t===77?"Snow Grains":t===80||t===81||t===82?"Rain Showers":t===85||t===86?"Snow Showers":t===95||t===96||t===99?"Thunderstorm":"Cloudy"}function Be(e,t,n="C"){const r=document.getElementById("chart-container");if(!r)return;r.innerHTML="";const o=document.createElementNS("http://www.w3.org/2000/svg","svg"),i=r.clientWidth||600,a=r.clientHeight||180;o.setAttribute("width","100%"),o.setAttribute("height","100%"),o.setAttribute("viewBox",`0 0 ${i} ${a}`),o.setAttribute("preserveAspectRatio","none"),o.style.overflow="visible",r.appendChild(o);const s=["6 AM","8 AM","10 AM","12 PM","2 PM","4 PM","6 PM","8 PM","10 PM"],d=["6 AM","","10 AM","","2 PM","4 PM","6 PM","","10 PM"],h=40,g=40,w=i-h*2,p=a-g*2,l=Math.min(...e)-1.5,f=Math.max(...e)+1.5-l,v=e.map((m,b)=>{const A=h+b/(e.length-1)*w,y=g+(1-(m-l)/(f||1))*p;return{x:A,y,temp:m,time:s[b],displayTime:d[b]}}),L=document.createElementNS("http://www.w3.org/2000/svg","defs"),S=document.createElementNS("http://www.w3.org/2000/svg","linearGradient");S.setAttribute("id","strokeGrad"),S.setAttribute("x1","0%"),S.setAttribute("y1","0%"),S.setAttribute("x2","100%"),S.setAttribute("y2","0%");const _=document.createElementNS("http://www.w3.org/2000/svg","stop");_.setAttribute("offset","0%"),_.setAttribute("stop-color","#60A5FA");const F=document.createElementNS("http://www.w3.org/2000/svg","stop");F.setAttribute("offset","100%"),F.setAttribute("stop-color","#2563EB"),S.appendChild(_),S.appendChild(F);const D=document.createElementNS("http://www.w3.org/2000/svg","linearGradient");D.setAttribute("id","fillGrad"),D.setAttribute("x1","0%"),D.setAttribute("y1","0%"),D.setAttribute("x2","0%"),D.setAttribute("y2","100%");const M=document.createElementNS("http://www.w3.org/2000/svg","stop");M.setAttribute("offset","0%"),M.setAttribute("stop-color","#3B82F6"),M.setAttribute("stop-opacity",t==="dark"?"0.3":"0.15");const O=document.createElementNS("http://www.w3.org/2000/svg","stop");O.setAttribute("offset","100%"),O.setAttribute("stop-color","#3B82F6"),O.setAttribute("stop-opacity","0.00"),D.appendChild(M),D.appendChild(O),L.appendChild(S),L.appendChild(D),o.appendChild(L),v.forEach(m=>{const b=document.createElementNS("http://www.w3.org/2000/svg","line");b.setAttribute("x1",m.x),b.setAttribute("y1",g),b.setAttribute("x2",m.x),b.setAttribute("y2",a-g+10),b.setAttribute("stroke",t==="dark"?"#1E293B":"#E2E8F0"),b.setAttribute("stroke-dasharray","4,4"),b.setAttribute("stroke-width","1"),o.appendChild(b)});let U=`M ${v[0].x} ${v[0].y}`;for(let m=0;m<v.length-1;m++){const b=v[m],A=v[m+1],y=b.x+(A.x-b.x)/3,H=b.y,x=b.x+2*(A.x-b.x)/3,c=A.y;U+=` C ${y} ${H}, ${x} ${c}, ${A.x} ${A.y}`}const G=document.createElementNS("http://www.w3.org/2000/svg","path"),N=`${U} L ${v[v.length-1].x} ${a-g} L ${v[0].x} ${a-g} Z`;G.setAttribute("d",N),G.setAttribute("fill","url(#fillGrad)"),o.appendChild(G);const E=document.createElementNS("http://www.w3.org/2000/svg","path");E.setAttribute("d",U),E.setAttribute("fill","none"),E.setAttribute("stroke","url(#strokeGrad)"),E.setAttribute("stroke-width","3"),E.setAttribute("stroke-linecap","round"),o.appendChild(E);let I=document.getElementById("chart-tooltip");I||(I=document.createElement("div"),I.id="chart-tooltip",I.className="chart-tooltip",r.appendChild(I)),v.forEach((m,b)=>{if(m.displayTime){const c=document.createElementNS("http://www.w3.org/2000/svg","text");c.setAttribute("x",m.x),c.setAttribute("y",a-12),c.setAttribute("text-anchor","middle"),c.setAttribute("fill",t==="dark"?"#64748B":"#94A3B8"),c.setAttribute("font-size","11"),c.setAttribute("font-family","var(--font-sans)"),c.setAttribute("font-weight","500"),c.textContent=m.displayTime,o.appendChild(c)}const A=document.createElementNS("http://www.w3.org/2000/svg","text");A.setAttribute("x",m.x),A.setAttribute("y",m.y-12),A.setAttribute("text-anchor","middle"),A.setAttribute("fill",t==="dark"?"#94A3B8":"#64748B"),A.setAttribute("font-size","11"),A.setAttribute("font-family","var(--font-sans)"),A.setAttribute("font-weight","600"),A.textContent=`${m.temp}°`,o.appendChild(A);const y=document.createElementNS("http://www.w3.org/2000/svg","circle");y.setAttribute("cx",m.x),y.setAttribute("cy",m.y),y.setAttribute("r","4.5"),y.setAttribute("fill",t==="dark"?"#0F172A":"#FFFFFF"),y.setAttribute("stroke","#3B82F6"),y.setAttribute("stroke-width","2.5"),y.setAttribute("class","chart-dot-node"),y.style.transition="all 0.15s ease",o.appendChild(y);const H=w/(e.length-1),x=document.createElementNS("http://www.w3.org/2000/svg","rect");x.setAttribute("x",m.x-H/2),x.setAttribute("y",g-10),x.setAttribute("width",H),x.setAttribute("height",p+20),x.setAttribute("fill","transparent"),x.style.cursor="pointer",x.addEventListener("mouseenter",c=>{y.setAttribute("r","6.5"),y.setAttribute("stroke-width","3.5"),y.setAttribute("fill","#3B82F6"),I.innerHTML=`
    ...
    `)}function Fe(e,t){const n=document.getElementById("weather-dashboard-content");if(n){n.innerHTML=`
    ...
    `,le(e,"hero-panel"),ce(e,"air-conditions-container"),de(e.forecast,"forecast-list-container"),He(e,"stats-grid-container"),Pe(t,e,"saved-cities-container");const i=localStorage.getItem("temp_unit")||"C",a=e.chartData.map(d=>$(d));Be(a,n,i);const s=document.getElementById("add-active-city-btn");s&&(t.some(h=>h.name.toLowerCase()===e.name.toLowerCase())?(s.style.opacity="0.4",s.style.cursor="not-allowed",s.title="City already saved"):s.addEventListener("click",()=>{R.onAddCity({name:e.name,lat:e.lat,lon:e.lon,country:e.country,timezone:e.timezone||"auto"})}))}const o=document.getElementById("dashboard-weather-widget");o&&(o.innerHTML=`
    ...
    <div class="hero-card">
      ...
      <div class="hero-center-row">
        ...
        <div class="hero-condition-details">
          ...
          <span class="hero-high-low">H:${L}° L:${S}°</span>
        </div>
      </div>
  ...
  `}function ce(e,t){const n=document.getElementById(t);n&&(n.innerHTML=`
    <!-- AQI card -->
  ...
  `)}function de(e,t){const n=document.getElementById(t);n&&(n.innerHTML=e.map((r,o)=>`
  ...
  `}function Pe(e,t,n){const r=document.getElementById(n);if(r){if(e.length===0){r.innerHTML='<div style="grid-column: span 2; text-align: center; color: var(--color-text-secondary); font-size: 13px; padding: 20px;">No saved cities. Search and pin one!</div>';return}r.innerHTML=e.map(o=>{const i=o.name.toLowerCase()===t.name.toLowerCase();let a="--°",s="Fetch data";if(i)a=`${$(t.temp)}°`,s=t.condition;else{const h={"new york":{temp:25,desc:"Clear Skies"},london:{temp:14,desc:"Light Rain"},tokyo:{temp:28,desc:"Sunny"},"san francisco":{temp:18,desc:"Partly Cloudy"}}[o.name.toLowerCase()];h?(a=`${$(h.temp)}°`,s=h.desc):(a=`${$(16)}°`,s="Partly Cloudy")}return`
    ...
    `}).join(""),r.querySelectorAll(".saved-city-item").forEach(o=>{o.addEventListener("click",i=>{if(i.target.closest(".saved-city-delete-btn"))return;const a={name:o.getAttribute("data-name"),lat:parseFloat(o.getAttribute("data-lat")),lon:parseFloat(o.getAttribute("data-lon")),country:o.getAttribute("data-country"),timezone:o.getAttribute("data-timezone")};R.onCitySelect(a)})}),r.querySelectorAll(".saved-city-delete-btn").forEach(o=>{o.addEventListener("click",i=>{i.stopPropagation();const a=o.getAttribute("data-name"),s=o.getAttribute("data-country");R.onDeleteCity(a,s)})})}}function Oe(e){if(e==null)return"SW direction";const t=Math.floor(e/45+.5);return["N direction","NE direction","E direction","SE direction","S direction","SW direction","W direction","NW direction"][t%8]}function ze(e){return e<=50?{text:"GOOD",class:"text-good",bgClass:"status-good-bg"}:e<=100?{text:"MODERATE",class:"text-moderate",bgClass:"status-moderate-bg"}:e<=150?{text:"UNHEALTHY (SG)",class:"text-unhealthy",bgClass:"status-unhealthy-bg"}:{text:"UNHEALTHY",class:"text-unhealthy",bgClass:"status-unhealthy-bg"}}function We(e){return e<=2?{text:"LOW",class:"text-good",bgClass:"status-good-bg"}:e<=5?{text:"MODERATE",class:"text-moderate",bgClass:"status-moderate-bg"}:e<=7?{text:"HIGH",class:"text-high",bgClass:"status-high-bg"}:{text:"VERY HIGH",class:"text-extreme",bgClass:"status-extreme-bg"}}function Ue(e){return e<1009?{text:"LOW",class:"text-low",bgClass:"status-low-bg"}:e>1020?{text:"HIGH",class:"text-high",bgClass:"status-high-bg"}:{text:"NORMAL",class:"text-normal",bgClass:"status-normal-bg"}}async function Ge(e){if(!e||e.trim().length<2)return[];try{const t=`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(e)}&count=5&language=en&format=json`,n=await fetch(t);if(!n.ok)throw new Error("Geocoding failed");return(await n.json()).results||[]}catch(t){return console.error("Geocoding API error:",t),[]}}async function je(e,t,n,r="US",o="auto"){const i=encodeURIComponent(o),a=`https://api.open-meteo.com/v1/forecast?latitude=${e}&longitude=${t}&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,surface_pressure,wind_speed_10m,wind_direction_10m,visibility&hourly=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=${i}`,s=`https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${e}&longitude=${t}&current=us_aqi,uv_index&timezone=${i}`;try{const[d,h]=await Promise.allSettled([fetch(a).then(c=>{if(!c.ok)throw new Error(`HTTP error ${c.status}`);return c.json()}),fetch(s).then(c=>{if(!c.ok)throw new Error(`HTTP error ${c.status}`);return c.json()})]);if(d.status==="rejected")throw new Error("Failed to fetch core weather data: "+d.reason.message);const g=d.value,w=h.status==="fulfilled"?h.value:null,p=g.current,l=g.hourly,u=g.daily,f=Math.round(p.temperature_2m),v=Math.round(p.apparent_temperature),L=Math.round(p.relative_humidity_2m),S=Math.round(p.wind_speed_10m),_=Oe(p.wind_direction_10m),F=p.visibility?Math.round(p.visibility/1e3):16,D=Math.round(p.surface_pressure),M=p.weather_code,O=se(M),U=u&&u.temperature_2m_max?Math.round(u.temperature_2m_max[0]):f+3,G=u&&u.temperature_2m_min?Math.round(u.temperature_2m_min[0]):f-4;let N=42,E=5;w&&w.current?(N=Math.round(w.current.us_aqi||42),E=Math.round(w.current.uv_index||5)):M===0?(E=f>25?8:5,N=35):[1,2,3].includes(M)?(E=f>25?6:4,N=40):(E=2,N=20);const I=ze(N),m=We(E),b=Ue(D),y=[6,8,10,12,14,16,18,20,22].map(c=>l&&l.temperature_2m&&l.temperature_2m[c]!==void 0?Math.round(l.temperature_2m[c]):Math.round(f+Math.sin((c-14)/4)*4)),H=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],x=[];if(u&&u.time)for(let c=0;c<Math.min(u.time.length,7);c++){const q=new Date(u.time[c]),z=H[q.getDay()],j=Math.round(u.temperature_2m_max[c]),ke=Math.round(u.temperature_2m_min[c]),ne=u.weather_code[c];x.push({day:z,desc:se(ne),icon:fe(ne),max:j,min:ke})}else{const c=new Date;for(let q=0;q<7;q++){const z=new Date(c);z.setDate(c.getDate()+q);const j=H[z.getDay()];x.push({day:j,desc:"Partly Cloudy",icon:"cloud",max:f+2,min:f-3})}}return{name:n,country:r.toUpperCase(),lat:e,lon:t,temp:f,condition:O,feelsLike:v,high:U,low:G,humidity:L,windSpeed:S,windDirection:_,visibility:F,aqi:N,aqiStatus:I.text,aqiClass:I.class,aqiBg:I.bgClass,uv:E,uvStatus:m.text,uvClass:m.class,uvBg:m.bgClass,pressure:D,pressureStatus:b.text,pressureClass:b.class,pressureBg:b.bgClass,chartData:y,forecast:x,weatherCode:M}}catch(d){throw console.error("fetchWeatherData orchestration error:",d),d}}const be="weather_dashboard_saved_cities",ye="weather_dashboard_active_city",ue=[{name:"San Francisco",lat:37.7749,lon:-122.4194,country:"US"},{name:"New York",lat:40.7128,lon:-74.006,country:"US"},{name:"London",lat:51.5074,lon:-.1278,country:"GB"},{name:"Tokyo",lat:35.6762,lon:139.6503,country:"JP"}],he={name:"San Francisco",lat:37.7749,lon:-122.4194,country:"US"},X={};function Ve(){const e=localStorage.getItem(ye);try{return e?JSON.parse(e):he}catch{return he}}function Je(e){!e||!e.name||localStorage.setItem(ye,JSON.stringify(e))}function B(){const e=localStorage.getItem(be);try{return e?JSON.parse(e):ue}catch{return ue}}function Ae(e){localStorage.setItem(be,JSON.stringify(e))}function Ye(e){if(!e||!e.name)return;const t=B();t.some(r=>r.name.toLowerCase()===e.name.toLowerCase()&&r.country.toLowerCase()===e.country.toLowerCase())||(t.push(e),Ae(t))}function Qe(e,t){let n=B();n=n.filter(r=>!(r.name.toLowerCase()===e.toLowerCase()&&r.country.toLowerCase()===t.toLowerCase())),Ae(n)}function Ke(e,t){X[e.toLowerCase()]={timestamp:Date.now(),data:t}}function Ze(e){const t=X[e.toLowerCase()];return t?Date.now()-t.timestamp>900*1e3?(delete X[e.toLowerCase()],null):t.data:null}const Xe=!0;let ee="All",te="",T=[],Y=null,Q=null,K=0,Z=null,C=null;function oe(e){const t=document.getElementById("article-count");t&&(t.textContent=`${e} article${e!==1?"s":""}`),Y=new Date,me(),Q&&clearInterval(Q),Q=setInterval(me,6e4)}function me(){const e=document.getElementById("last-updated");e&&Y&&(Math.floor((new Date-Y)/1e3)<60?e.textContent="Updated just now":e.textContent=`Updated ${window.timeAgo?window.timeAgo(Y.toISOString()):"recently"}`)}function Se(e,t){!e||e.length===0||e.sort((n,r)=>{if(t==="newest")return new Date(r.publishedAt||0)-new Date(n.publishedAt||0);if(t==="oldest")return new Date(n.publishedAt||0)-new Date(r.publishedAt||0);if(t==="popular"){const o=n.popularity||(n.title?n.title.length:0);return(r.popularity||(r.title?r.title.length:0))-o}return 0})}function ge(e){const t=document.getElementById("debug-badge");t&&(t.style.display="inline-block",e==="live"?(t.textContent="Live API",t.style.backgroundColor="#10b981"):e==="mock"?(t.textContent="Mock Data",t.style.backgroundColor="#f59e0b"):t.style.display="none")}async function V(e=""){window.currentView="news";const t=document.getElementById("saved-articles-btn");t&&(t.style.color="inherit");const n=document.querySelector("#news-view .news-header-titles h2");n&&(n.textContent="News Feed"),te=e,console.log(`[News Flow] handleLocationNews started for: "${e}". Category: "${ee}".`),window.showNewsLoading("news-grid"),ge(null);const r=++K;try{const o=await window.getNews({query:e,category:ee});if(r!==K)return;if(ge(o._source),o&&o.articles&&o.articles.length>0){T=o.articles;const i=document.getElementById("news-sort"),a=i?i.value:"newest";Se(T,a),window.renderNewsCards(T,"news-grid"),oe(T.length)}else T=[],window.showNoNewsFound(e,"news-grid")}catch(o){if(r!==K)return;T=[],o.message&&o.message.includes("No results")?window.showNoNewsFound(e,"news-grid"):window.showNewsError(o.message||"An error occurred while fetching news.",()=>V(e),"news-grid")}}async function et(e=""){const t="dashboard-news-grid",n=document.getElementById("dashboard-article-count"),r=document.getElementById("dashboard-debug-badge");window.showNewsLoading(t),r&&(r.style.display="none");try{const o=await window.getNews({query:e,category:"All"});if(o&&o.articles&&o.articles.length>0){const i=o.articles.slice(0,4);window.renderNewsCards(i,t),n&&(n.textContent=`${o.articles.length} article${o.articles.length!==1?"s":""}`),r&&Xe&&(r.style.display="inline-block",o._source==="live"?(r.textContent="Live API",r.style.backgroundColor="#10b981"):(r.textContent="Mock Data",r.style.backgroundColor="#f59e0b"))}else window.showNoNewsFound(e,t),n&&(n.textContent="0 articles")}catch{window.showNoNewsFound(e,t),n&&(n.textContent="0 articles")}}async function W(e){_e();try{const t=Ze(e.name);if(t)C=t;else{const r=await je(e.lat,e.lon,e.name,e.country||"US",e.timezone||"auto");Ke(e.name,r),C=r}Je(e);const n=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";P(C,B(),n),et(e.name),V(e.name)}catch(t){console.error("Error loading weather data:",t),Fe(`Could not retrieve weather data for ${e.name}. Please check your connection.`,()=>W(e))}}function tt(e){Z&&clearTimeout(Z),Z=setTimeout(async()=>{if(e.length<2)return;const t=await Ge(e);Re(t)},300)}function ot(e){Ye(e);const t=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";C&&P(C,B(),t)}function rt(e,t){Qe(e,t);const n=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";C&&P(C,B(),n)}function pe(){const t=(document.documentElement.getAttribute("data-theme")==="light"?"light":"dark")==="light"?"dark":"light";t==="light"?(document.documentElement.setAttribute("data-theme","light"),localStorage.setItem("theme","light")):(document.documentElement.removeAttribute("data-theme"),localStorage.setItem("theme","dark")),ve(t),C&&P(C,B(),t)}function nt(){const e=document.querySelectorAll(".nav-tab");e.forEach(t=>{t.addEventListener("click",()=>{e.forEach(o=>o.classList.remove("active")),t.classList.add("active");const n=t.dataset.tab;document.querySelectorAll(".tab-content").forEach(o=>{o.classList.remove("active")});const r=document.getElementById(`${n}-view`);r&&r.classList.add("active"),(n==="weather"||n==="dashboard")&&C&&setTimeout(()=>{const o=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";P(C,B(),o)},50)})})}document.addEventListener("DOMContentLoaded",()=>{Ne({onSearchInput:tt,onCitySelect:W,onThemeToggle:pe,onAddCity:ot,onDeleteCity:rt}),nt();const e=localStorage.getItem("theme"),t=window.matchMedia&&window.matchMedia("(prefers-color-scheme: light)").matches,n=e==="light"||!e&&t?"light":"dark";n==="light"?document.documentElement.setAttribute("data-theme","light"):document.documentElement.removeAttribute("data-theme"),ve(n);const r=document.getElementById("theme-toggle");r&&r.addEventListener("click",pe);let o=localStorage.getItem("temp_unit")||"C";const i=document.getElementById("temp-toggle-btn");i&&(i.textContent=`°${o}`,i.addEventListener("click",()=>{if(o=o==="C"?"F":"C",localStorage.setItem("temp_unit",o),i.textContent=`°${o}`,C){const l=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";P(C,B(),l)}}));const a=Ve();navigator.geolocation?navigator.geolocation.getCurrentPosition(async l=>{const u=l.coords.latitude,f=l.coords.longitude;try{const v=`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${u}&longitude=${f}&localityLanguage=en`,L=await fetch(v);if(!L.ok)throw new Error("Reverse geocoding failed");const S=await L.json(),_=S.city||S.locality||S.principalSubdivision||"My Location",F=S.countryCode||"US";W({name:_,lat:u,lon:f,country:F,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||"auto"})}catch(v){console.warn("Reverse geocoding failed, falling back to default city:",v),W(a)}},l=>{console.warn("Geolocation permission denied or failed, falling back to default city:",l),W(a)},{timeout:1e4}):W(a);const s=document.getElementById("news-search");let d;s&&s.addEventListener("input",l=>{clearTimeout(d),d=setTimeout(()=>{const u=l.target.value.trim();V(u||a.name)},300)});const h=document.querySelectorAll(".category-filters .filter-btn");h.forEach(l=>{l.addEventListener("click",u=>{h.forEach(v=>v.classList.remove("active")),u.target.classList.add("active"),ee=u.target.dataset.category||"All";const f=s&&s.value.trim()?s.value.trim():te;V(f)})});const g=document.getElementById("news-sort");g&&g.addEventListener("change",l=>{T.length>0&&(Se(T,l.target.value),window.renderNewsCards(T,"news-grid"),oe(T.length))});const w=document.getElementById("saved-articles-btn");w&&w.addEventListener("click",()=>{if(window.currentView=window.currentView==="saved"?"news":"saved",window.currentView==="saved"){const l=document.querySelector('.nav-tab[data-tab="news"]');l&&l.click(),w.style.color="var(--accent)",T=JSON.parse(localStorage.getItem("saved_articles")||"[]"),window.renderNewsCards(T,"news-grid"),oe(T.length);const f=document.querySelector("#news-view .news-header-titles h2");f&&(f.textContent="Saved Articles")}else{w.style.color="inherit";const l=s&&s.value.trim()?s.value.trim():te,u=document.querySelector("#news-view .news-header-titles h2");u&&(u.textContent="News Feed"),V(l)}});let p=null;window.addEventListener("resize",()=>{p&&clearTimeout(p),p=setTimeout(()=>{if(C){const l=document.documentElement.getAttribute("data-theme")==="light"?"light":"dark";P(C,B(),l)}},200)})});
