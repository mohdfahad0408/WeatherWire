/* ==========================================================================
   WEATHER DASHBOARD ICON LIBRARY (icons.js)
   ========================================================================== */
...
export function getIcon(name, customClass = "") {
  const svg = ICONS[name] || ICONS.cloud;
  if (customClass) {
    return svg.replace(/class="([^"]*)"/, `class="$1 ${customClass}"`);
  }
  return svg;
}

export function getWeatherIconByWmoCode(code) {
  // WMO weather code mapping to ICONS keys
  if (code === 0) return "sun"; // Clear sky
  if ([1, 2, 3].includes(code)) return "cloud"; // Mainly clear, partly cloudy, overcast
  if ([45, 48].includes(code)) return "cloud"; // Foggy
  if ([51, 53, 55, 56, 57].includes(code)) return "cloud-drizzle"; // Drizzle
  if ([61, 63, 65, 66, 67, 80, 81, 82].includes(code)) return "cloud-rain"; // Rain/Showers
  if ([71, 73, 75, 77, 85, 86].includes(code)) return "snowflake"; // Snow
  if ([95, 96, 99].includes(code)) return "cloud-lightning"; // Thunderstorm
  return "cloud";
}

export function getWeatherDescByWmoCode(code) {
