 ...
 */
function showNewsError(message, retryCallback) {
    const grid = document.getElementById('news-grid');
    if (!grid) return;
 ...
 */
function showNoNewsFound(query) {
    const grid = document.getElementById('news-grid');
    if (!grid) return;
...
/**
 * Sets up event delegation for bookmark buttons in the news grid.
 */
...
// Initialize event delegation (safe to call here because of defer attribute on script tag)
setupBookmarkDelegation();
window.showNewsLoading = showNewsLoading; window.showNewsError = showNewsError; window.showNoNewsFound = showNoNewsFound; window.renderNewsCards = renderNewsCards;
