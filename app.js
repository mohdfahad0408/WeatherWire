 ...
 */
async function handleLocationNews(cityName = '', countryCode = '') {
    ...
    console.log(`[News Flow] handleLocationNews started for: "${cityName}" (Country: "${countryCode}"). Category: "${currentCategory}".`);
    ...
    try {
        const queryToUse = countryCode ? '' : cityName;
        const data = await window.getNews({ query: queryToUse, country: countryCode, category: currentCategory });
        ...
        if (data && data.articles && data.articles.length > 0) {
            ...
            currentArticles = [];
            window.showNoNewsFound(cityName || countryCode, 'news-grid');
        }
    } catch (error) {
        if (requestId !== currentRequestId) return;
        currentArticles = [];
        ...
        if (error.message && error.message.includes('No results')) {
            ...
            window.showNewsError(error.message || 'An error occurred while fetching news.', () => handleLocationNews(cityName, countryCode), 'news-grid');
        }
    }
}
 ...
 * @param {string} cityName 
 */
async function fetchDashboardNews(cityName = '', countryCode = '') {
    ...
    try {
        const queryToUse = countryCode ? '' : cityName;
        const data = await window.getNews({ query: queryToUse, country: countryCode, category: 'All' });
        ...
        if (data && data.articles && data.articles.length > 0) {
            ...
            window.showNoNewsFound(cityName || countryCode, gridId);
...
async function loadCityWeather(cityObj) {
  ...
  try {
    ...
    fetchDashboardNews(cityObj.name, cityObj.country);
    handleLocationNews(cityObj.name, cityObj.country);
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        newsSearch.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(() => {
                ...
                if (inputValue) {
                    ...
                    handleLocationNews('', activeCity ? activeCity.country : '');
                }
            }, 300);
        });
    }
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const queryToUse = newsSearch && newsSearch.value.trim() ? newsSearch.value.trim() : '';
            const countryToUse = queryToUse ? '' : (activeCity ? activeCity.country : '');
            handleLocationNews(queryToUse, countryToUse);
        });
    });
